...
RSpec.describe TagsafeJsEventsConsumers::NewTags do
  before(:each) do
    prepare_test!
  ...
  describe '#consume!' do
    ...
    it 'creates new tags from the provided `new_tags` data if they do not yet exist' do
      expect(@domain.tags.count).to be(1)
      expect(@domain.tags.collect(&:full_url)).to_not include('https://cdn.example.com/script.js')
      
      @consumer.consume!

      @domain.tags.reload
      expect(@domain.tags.count).to be(2)
      expect(@domain.tags.collect(&:full_url)).to include('https://cdn.example.com/script.js')
    end

    it 'does not create a new tag from if the URL in the `new_tags` data already exists' do
      expect(@domain.tags.count).to be(1)
      @data['new_tags'][0]['tag_url'] = @domain.tags.first.full_url
    ...
    it 'adds the new Tag URL to the domain\'s `tag_url_patterns_to_not_capture` if it exceeds the max number of tags for that URL' do
      ...
      expect(@domain.tags.count).to be(1)
      expect(@domain.tag_url_patterns_to_not_capture.count).to be(0)
      
      @data['new_tags'][0]['tag_url'] = "#{@domain.tags.first.full_url}?a_query_param_that_makes_this_a_new_url=true"
      TagsafeJsEventsConsumers::NewTags.new(@data).consume!

      @domain.tags.reload
      expect(@domain.tags.count).to be(1)
      expect(@domain.tag_url_patterns_to_not_capture.collect(&:url_pattern)).to eq(["#{@tag.url_domain}#{@tag.url_path}"])
    end
    ...
    it 're-builds instrumentation if a new tag was added' do
      ...
      @consumer.consume!
      expect(@domain.reload.tags.count).to eq(2)
    end
    ...
    it 're-builds instrumentation if a `tag_url_pattern_to_not_capture` was added' do
      ...
      @domain.reload
      expect(@domain.tags.count).to be(1)
      expect(@domain.tag_url_patterns_to_not_capture.count).to be(1)
    end
    ...
    it 'does not re-build instrumentation if a new tag was not added and a `tag_url_pattern_to_not_capture` was not added' do
      ...
      expect(@domain.tags.count).to eq(1)
      expect(@domain.tag_url_patterns_to_not_capture.count).to eq(0)
