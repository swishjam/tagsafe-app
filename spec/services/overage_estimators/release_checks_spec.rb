...
RSpec.describe OverageEstimators::ReleaseChecks do
  before(:each) do
    ...
    @subscription_feature_restriction = create(:subscription_feature_restriction_zero_features, release_checks_included_per_month: num_release_checks_at_every_12_hours, domain: @domain)
  ...
  describe '#would_exceed_included_usage_for_subscription_package?' do
    ...
    it 'returns false when the new release_check_minute_interval would result in the exact ReleaseChecks alloted for their Subscription package' do
      ...
      exact_num_release_checks_per_day_for_subscription_package = (@domain.subscription_feature_restriction.release_checks_included_per_month / minutes_in_next_month).round
  ...
  describe '#expected_dollar_overage_next_month_on_current_config' do
    it 'returns the cost of the domain\'s current release check configuration, excluding the included_release_checks_per_month in their subscription_feature_restriction' do
      @tag.tag_preferences.update_column :release_check_minute_interval, 1
      overage_estimator = OverageEstimators::ReleaseChecks.new(domain: @domain, tag: @tag, new_release_check_interval: 1_440)
      expected_num_release_checks_next_month = (Time.current.next_month.end_of_month - Time.current.next_month.beginning_of_month) / 1.minute
      expected_cost_of_release_checks_next_month = (expected_num_release_checks_next_month - @domain.subscription_feature_restriction.release_checks_included_per_month)*OverageEstimators::ReleaseChecks::COST_PER_RELEASE_CHECK_IN_DOLLARS
