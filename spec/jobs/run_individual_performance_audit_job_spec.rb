...
RSpec.describe RunIndividualPerformanceAuditJob do
  ...
  describe '#perform_later' do
    ...
    it 'calls `send!` on an instance of the `lambda_sender_class`' do
      ...
      expect_any_instance_of(LambdaModerator::Senders::PerformanceAuditerWithTag).to receive(:send!).exactly(:once).and_return(success_response)
      perform_job
    end
    ...
    it 'calls `capture_successful_response` when the response of the sender `send!` call is successful' do
      ...
      expect_any_instance_of(LambdaModerator::Senders::PerformanceAuditerWithTag).to receive(:send!).exactly(:once).and_return(successful_response)
    ...
    it 'calls `error!` on the lambda_sender_class individual_performance_audit when the response of the sender `send!` call is unsuccessful' do
      ...
      expect_any_instance_of(LambdaModerator::Senders::PerformanceAuditerWithTag).to receive(:send!).exactly(:once).and_return(unsuccessful_response)
