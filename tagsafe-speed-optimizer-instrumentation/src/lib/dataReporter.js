export default class DataReporter {
  constructor({ reportingURL, containerUid, reportingSampleRate = 1, debugMode = false }) {
    ...
    if (this.reportingEnabled) { 
      if(this.debugMode) console.log(`%c[Tagsafe Log] Reporting is enabled (sample rate is set to ${reportingSampleRate * 100}%).`, 'background-color: #7587f8; color: white; padding: 5px;');
      this._startReportingInterval();
      window.Tagsafe.config.reportingEnabled = true;
    } else {
      if (this.debugMode) console.log(`%c[Tagsafe Log] Reporting is disabled (sample rate is set to ${reportingSampleRate * 100}%).`, 'background-color: #7587f8; color: white; padding: 5px;');
      window.Tagsafe.config.reportingEnabled = false;
    }
  }

  recordThirdPartyTag({ tagUrl, loadType, interceptedByTagsafeJs, hostedByTagsafe }) {
    ...
    this._recordData('third_party_tags', { 
      ...
      hosted_by_tagsafe: hostedByTagsafe
    });
  }
  ...
  //   if(this.debugMode) {
  //     console.log(`%c[Tagsafe Log] recording num_tagsafe_injected_tags: ${numTags}`, 'background-color: #7587f8; color: white; padding: 5px;');
  //   }
  // }
  ...
  recordNumTagsafeHostedTags(numTags) {
    ...
    if(this.debugMode) {
      console.log(`%c[Tagsafe Log] recording num_tagsafe_hosted_tags: ${numTags}`, 'background-color: #7587f8; color: white; padding: 5px;');
    }
  }
  ...
  recordNumTagsWithTagsafeOverriddenLoadStrategies(numTags) {
    ...
    if (this.debugMode) {
      console.log(`%c[Tagsafe Log] recording num_tags_with_tagsafe_overridden_load_strategies: ${numTags}`, 'background-color: #7587f8; color: white; padding: 5px;');
    }
  }
  ...
  recordNumTagsNotHostedByTagsafe(numTags) {
    ...
    if(this.debugMode) {
      console.log(`%c[Tagsafe Log] recording num_tags_not_hosted_by_tagsafe: ${numTags}`, 'background-color: #7587f8; color: white; padding: 5px;');
    }
  }
  ...
  recordPerformanceMetric(metricName, data) {
    ...
    if(this.debugMode && this.reportingEnabled) {
      console.log(`%c[Tagsafe Log] New reporting data to report: ${metricName}`, 'background-color: #7587f8; color: white; padding: 5px;');
      console.log(data);
    }
  }
  ...
  _recordData(type, data) {
    ...
    this.lastReceivedDataAt = Date.now();
    if(this.debugMode && this.reportingEnabled) {
      console.log(`%c[Tagsafe Log] New ${type} data to report to Tagsafe API`, 'background-color: #7587f8; color: white; padding: 5px;');
      console.log(data);
    }
  }
  ...
  async _reportPendingData() {
    try {
      ...
      if(this.debugMode) {
        console.log(`%c[Tagsafe Log] Sending data to Tagsafe API`, 'background-color: #7587f8; color: white; padding: 5px;');
        console.log(body);
      }
