class DataRetentionEnforcer
  ...
  def purge_tag_versions!
    if @domain.tag_versions.count == 1
      ...
      tag_versions_to_purge = @domain.tag_versions.older_than(Time.current - @days_of_retention_for_domain.days, timestamp_column: :'tag_versions.created_at')
  ...
  def purge_audits!
    audits_to_purge = @domain.audits.older_than(Time.current - @days_of_retention_for_domain.days, timestamp_column: :'audits.created_at')
