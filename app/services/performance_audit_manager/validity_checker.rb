  ...
  class ValidityChecker
    ...
    def valid?
      if !blocked_tag_in_audit_without_tag
        invalid_reason = "One or more of the performance audits did not block the audited tag when it should have.\ 
                          Check to ensure the tag is present on this page, or if it's deployed via a tag manager ensure \ 
                          the tag manager is an allowed tag in the tag's performance audit settings."
        false
      elsif !overwrote_tag_in_audit_with_tag
        invalid_reason = "One or more of the performance audits did not allow the audited tag when it should have.\ 
                          Check to ensure the tag is present on this page, or if it's deployed via a tag manager ensure \ 
                          the tag manager is an allowed tag in the tag's performance audit settings."
        false
      else
