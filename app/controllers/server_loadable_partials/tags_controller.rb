module ServerLoadablePartials
  class TagsController < BaseController
    def index
      if params[:q]
        tags = current_domain.tags.is_third_party_tag.still_on_site
                                    .includes(:tag_preferences, :tag_identifying_data, most_current_audit: :average_delta_performance_audit).joins(:tag_identifying_data)
        ...
        tags = current_domain.tags.is_third_party_tag.still_on_site
                                    .includes(:tag_preferences, :tag_identifying_data, most_current_audit: :average_delta_performance_audit)
                                    .order(last_audit_began_at: :DESC, removed_from_site_at: :ASC, last_released_at: :DESC)
                                    .page(params[:page] || 1).per(params[:per_page] || 9)
      end
