class DomainsController < LoggedInController
  ...
  def update
    ...
    if domain.update(domain_params)
      domain.scan_and_capture_domains_tags(initial_scan: true)
      display_toast_message("Scanning #{domain.url} for third party tags.")
    else
      display_toast_error(domain.errors.full_messages.join(' '))
    end
  ...
  def scan
    domain = Domain.find(params[:id])
    raise StandardError, 'No permission' unless domain.user_can_initiate_scan?(current_user)
    domain.scan_and_capture_domains_tags
