    ...
    def toggle_active
      if @script_subscriber.still_on_site?
        ...
        if @script_subscriber.errors.any?
          ...
          render_msg(true, "You have successfully turned monitoring #{@script_subscriber.active ? 'on' : 'off'} for #{@script_subscriber.script.url}. Tag Safe will no longer monitor changes, tests, and audits for this tag.")
        end
      else
