class GeneralConfigurationController < LoggedInController
  def update
    ...
    if current_container.general_configuration.update(priv_params_for_attr_being_updated)
      current_user.broadcast_notification(message: "#{attr_being_update.split('_').map(&:capitalize).join(' ')} setting updated.")
    else
      current_user.broadcast_notification(message: "Unable to update setting: #{current_container.general_configuration.errors.join('. ')}")
    end
    head :ok
  end
end