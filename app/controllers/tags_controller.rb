class TagsController < LoggedInController
  ...
  def performance_audit_settings
    @tag = Tag.joins(:tag_preferences).find(params[:tag_id])
    permitted_to_view?(@tag)
    render_breadcrumbs(
      { text: 'Monitor Center', url: tags_path }, 
      { text: "#{@tag.try_friendly_name} Details", url: tag_path(@tag) },
  ...
  def tag_params
    params.require(:tag).permit(:friendly_name, :image, 
                                tag_preferences_attributes: [:throttle_minute_threshold, :monitor_changes, :is_third_party_tag, :allowed_third_party_tag, :should_run_audit])
  end
end