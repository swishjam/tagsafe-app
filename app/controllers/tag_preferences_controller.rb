class TagPreferencesController < LoggedInController
  def update
    ...
    if params[:tag_preference][:release_check_minute_interval] && 
        params[:tag_preference][:release_check_minute_interval].to_i != tag.tag_preferences.release_check_minute_interval
      handle_release_check_interval_update(tag)
    elsif params[:tag_preference][:scheduled_audit_minute_interval] && 
            params[:tag_preference][:scheduled_audit_minute_interval].to_i != tag.tag_preferences.scheduled_audit_minute_interval
      handle_scheduled_audit_interval_update(tag)
    end
  end
    ...
    if !params[:tag_preference][:price_increase_confirmed] &&
      ...
      stream_modal(
        ...
        locals: {
          ...
          included_in_plan: current_domain.subscription_feature_restriction.release_checks_included_per_month
        }
      )
    else
    ...
    if !params[:tag_preference][:price_increase_confirmed] &&
      ...
      stream_modal(
        ...
        locals: {
          ...
          path_to_update_form: 'overage_estimators/confirm_tag_preference_form',
          overage_estimator: overage_estimator,
          included_in_plan: current_domain.subscription_feature_restriction.automated_performance_audits_included_per_month
        }
      )
    else
      update_tag_preference_config(tag)
