class TagPreferencesController < LoggedInController
  def update
    ...
    attr_being_update = params[:tag_preference].keys[0]
    priv_params_for_attr_being_updated = params.require(:tag_preference).permit(attr_being_update.to_sym)
    tag_preference = tag.tag_preferences
    if tag_preference.update(priv_params_for_attr_being_updated)
      current_user.broadcast_notification(message: "Updated #{tag.try_friendly_name}", image: tag.try_image_url)
    else
