class UrlsToCrawlController < LoggedInController
  def create
    ...
    if url_to_crawl.save
      current_user.broadcast_notification(message: "#{url_to_crawl.url} added to scan list.")
    else
      current_user.broadcast_notification(message: "Unable to add #{url_to_crawl.url} to scan list. #{url_to_crawl.errors.full_messages.join(' ')}")
    end
  ...
  def destroy
    ...
    current_user.broadcast_notification(message: "#{url_to_crawl.url} removed from scan list.")
    render turbo_stream: turbo_stream.replace(
      "#{current_domain.id}_urls_to_crawl",
      partial: 'urls_to_crawl/index',
      locals: { domain: current_domain }
