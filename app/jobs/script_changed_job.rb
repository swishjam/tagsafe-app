class ScriptChangedJob < ApplicationJob
  ...
  def perform(script_change)
    unless script_change.first_change?
      script_change.script.script_change_notification_subscribers.each do |change_subscriber|
    ...
    script_change.script.script_subscribers.active.each do |script_subscriber|
